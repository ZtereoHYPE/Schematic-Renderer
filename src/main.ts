...
import { GeometryPlacer, BuildData } from './GeometryPlacer';
...
let raycastableObjects: THREE.InstancedMesh[] = [];
let currentSelection: THREE.InstancedMesh | null = null;
...
function onPointerMove( event: { clientX: number; clientY: number; } ) {
    ...
    if ( intersects.length > 0 ) {
        ...
        const mesh = closestIntersect.object as THREE.InstancedMesh;
        const id = closestIntersect.instanceId ?? 0;

        if (currentSelection == mesh && currentId == id) {
            return;
        }
        
        if (currentSelection != null)
            markSelected(currentSelection, currentId ?? 0, false);
        
        currentSelection = mesh;
        currentId = id;
        
        markSelected(currentSelection, currentId, true);
    } else {
        if (currentSelection != null) {
            markSelected(currentSelection, currentId ?? 0, false);
        }
...
function markSelected(mesh: THREE.InstancedMesh, id: number, selected: boolean) {
    let duration = 300;

    let sourceColor: THREE.Color = new THREE.Color(0x000000);
    if(mesh.instanceColor)
        mesh.getColorAt(id, sourceColor);

    let destinationColor = selected ? new THREE.Color(0x777777) : new THREE.Color(0x000000);

    new TWEEN.Tween( sourceColor )
        .to( destinationColor, duration)
        .easing( TWEEN.Easing.Exponential.Out )
        .onUpdate( (vColor) => {
            mesh.setColorAt(id, vColor);
