...
import { RaycastEvents, SchematicRenderer } from './renderer/SchematicRenderer';
import { Matrix4, Vector3 } from 'three';
import { unpackSchematic } from './adapters/SchematicUnpacker';
import { FollowCursorControls } from './controls/FollowCursorControls';
import { StaticControls } from './controls/StaticControls';
...
type Selection = {
    object: [THREE.InstancedMesh, number],
    oldLocation: THREE.Matrix4,
    currentAnimation: TWEEN.Tween<Matrix4> | null,
    isReturning: boolean
};
...
const schematic = unpackSchematic(data);
schemRend.setSchematic(schematic);

// const controls = new FollowCursorControls(schemRend.getCamera(), canvas);
// controls.target = new THREE.Vector3(schematic.getSizeX() / 2, schematic.getSizeY() / 2, schematic.getSizeZ() / 2);
// controls.distance = 25;

const controls = new StaticControls(schemRend.getCamera());
controls.setPosition(schematic.getSizeX() / 2 + 20, schematic.getSizeY() / 2 + 5, schematic.getSizeZ() / 2 + 20);
controls.setTarget(schematic.getSizeX() / 2, schematic.getSizeY() / 2, schematic.getSizeZ() / 2);
schemRend.setControls(controls);

console.log(schemRend.getCamera().position, schematic.getSizeX() / 2, schematic.getSizeY() / 2, schematic.getSizeZ() / 2);
...
schemRend.onRaycast(RaycastEvents.CLICKED, (mesh, index) => {
    ...
    let target = schemRend.getCamera()!.matrix.clone();

    // shift matrix location away from camera vector
    target.elements[12] += dirVector.x;
    target.elements[13] += dirVector.y;
    target.elements[14] += dirVector.z;
...
function animateCommitSlide(mesh: THREE.InstancedMesh, index: number) {
    ...
    let cameraRotation = schemRend.getCamera()!.matrix.clone();
    cameraRotation.elements[12] = 0;
    cameraRotation.elements[13] = 0;
    cameraRotation.elements[14] = 0;

    let dirVector = new THREE.Vector3(-2, 0, 0);
    dirVector.applyMatrix4(cameraRotation);

    let target = source.clone();

    // shift matrix location away from camera vector
    target.elements[12] += 41 * dirVector.x;
    target.elements[13] += 41 * dirVector.y;
    target.elements[14] += 41 * dirVector.z;
    ...
    schemRend.setSchematicLocation(dirVector.clone().multiplyScalar(-40), duration)
